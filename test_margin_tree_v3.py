...
def create_test_data(close, curr_bin_pct, prev_bin_pct, below_chg):
    ...
    prices = []
    p = bin_low - step * 5
    while p <= bin_high + step * 5:
        prices.append(p)
        p += step / 2
    
    # 计算下方价格点数量
    below_prices = [p for p in prices if p < close]
    n_below = len(below_prices)
    ...
    for p in prices:
        if bin_low <= p < bin_high:
            ...
            # 下方筹码：基础值 + 变化量
            curr_pcts.append(5.0 + below_chg / n_below)
        else:
            curr_pcts.append(5.0)
...
# 测试场景定义
test_cases = [
    ("紫金矿业", 31.58, 27.6, 9.0, 5.0, 2.0, "✅", False),
    ("绿色动力", 9.71, 60.6, 47.3, 3.0, 2.0, "✅", False),
    ("锡业股份", 43.19, 7.1, 1.8, 2.0, 0.5, "⚠️", False),
    ("招商银行", 39.5, 41.2, 43.6, -1.0, 0, "❌", False),
    ("中国神华", 49.5, 30.8, 31.8, 0.5, 0, "❌", False),
    ("中国重汽", 23.0, 44.6, 54.4, -5.0, 0, "❌", False),
    ("沃尔核材", 22.36, 40.4, 79.6, 15.0, 0, "❌", True),
    ("恐慌出逃", 25.0, 20.0, 40.0, 12.0, 0, "❌", True),
    ("大幅减少", 25.0, 20.0, 40.0, 5.0, 0, "❌", False),
]
...
for name, close, curr_bin_pct, prev_bin_pct, below_chg, expected_score, expected_label, expected_panic in test_cases:
    ...
    print(f"{name:8s} | 当前价 {close:>6.2f} | 分箱 [{bin_low:.0f}, {bin_high:.0f}) | chg={actual_chg:>+6.1f}% | 下方={actual_below_chg:>+6.1f}%")
