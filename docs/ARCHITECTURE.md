...
TradingAgents-AShare 是一个基于**多智能体协作**的 A 股/港股深度投研分析系统。系统模拟顶级投研机构的决策闭环，通过 14+ 名专业 Agent 的多空辩论与风控博弈，为投资者提供结构化的交易建议。

### 1.1 核心特性

- **多智能体协作**：14 名 Agent 分角色协作（分析师、研究员、交易员、风控）
- **多空辩论机制**：Bull/Bear 结构化辩论，Risk 三方风控审查
- **意图驱动交互**：自然语言输入自动识别标的、周期、关注点
- **双周期分析**：支持短线（short）和中线（medium）并行分析
- **实时流式输出**：SSE 流式推送 Agent 思考过程和最终报告
- **多模型支持**：OpenAI、Anthropic、Google、DeepSeek、Moonshot 等
...
| 分析引擎 | LangGraph + LangChain |
| LLM 客户端 | OpenAI / Anthropic / Google / DeepSeek 等 |
...
│                              前端层 (Frontend)                                │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │  Dashboard  │ │  Analysis   │ │   Reports   │ │  Settings   │           │
│  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘           │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │  Portfolio  │ │TrackingBoard│ │   Login     │ │  Feedback   │           │
│  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘           │
│                                                                             │
...
│  │                         api/main.py                                  │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │   │
│  │  │  Auth    │ │ Analyze  │ │  Jobs    │ │ Reports  │ │ Portfolio│  │   │
│  │  │ /login   │ │ /analyze │ │ /jobs    │ │ /reports │ │ /portfolio│  │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘  │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐               │   │
│  │  │ Settings │ │ Scheduled│ │ Dashboard│ │ Feedback │               │   │
│  │  │ /config  │ │ /scheduled│ │ /dashboard│ │ /feedback│              │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
...
### 3.4 数据层 (tradingagents/dataflows/)
...
### 3.5 LLM 客户端 (tradingagents/llm_clients/)
