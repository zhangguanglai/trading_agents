...
# 当前筹码：分箱内 20%，下方 50%，上方 10%
curr_pcts = []
for p in prices:
    if bin_low <= p < bin_high:
        curr_pcts.append(10.0)  # 分箱内：20% / 2 = 10%
    elif p < close:
        curr_pcts.append(50.0 / len([x for x in prices if x < close and not (bin_low <= x < bin_high)]))
    else:
        curr_pcts.append(0.1)

# 2周前筹码：分箱内 40%，下方 30%，上方 10%
prev_pcts = []
for p in prices:
    if bin_low <= p < bin_high:
        prev_pcts.append(20.0)  # 分箱内：40% / 2 = 20%
    elif p < close:
        prev_pcts.append(30.0 / len([x for x in prices if x < close and not (bin_low <= x < bin_high)]))
    else:
